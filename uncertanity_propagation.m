%%%%%This is the uncertanity propagation for the flux circulation
clear all
clc
...
%rob_fit=rob_Data{56}(1:length(both_direction));
I=find(both_direction1'==1);% & rob_fit==1);





R=rob_Data{3}(I); vr=rob_Data{29}(I);vt=rob_Data{30}(I);LT=rob_Data{4}(I);
nw=rob_Data{32}(I);
...
%   E=E(~I1); R=R(~I1);
% kI=find(E>0);E_pos=E(kI);R1=R(kI);
% k=0;
% k1=3;
% for i=3:k1:27
%     k=k+1;
%     E1=E_pos(R1>=i & R1<i+k1);
%     II=~isnan(E1);
%     mean_E_pos(k)=mean(E1(II));
% end
% k=0;
% figure(1)
% plot(3:k1:27,mean_E_pos,'r','linewidth',5)
% hold on
% kII=find(E<0);E_neg=E(kII);R2=R(kII);
% for i=3:k1:27
%     k=k+1;
%     E2=E_neg(R2>=i & R2<i+k1);
%     II=~isnan(E2);
%     mean_E_neg(k)=mean(E2(II));
% end
% plot(3:k1:27,mean_E_neg,'b','linewidth',5)
% net=mean_E_pos+mean_E_neg;
% plot(3:k1:27,net,'k','linewidth',5)
% grid on
% xlim([8 26])
...
vr=vr(I1); E=E(I1); R=R(I1);vt=vt(I1); B_r_rel=B_r_rel(I1);R=R(I1);
B_t_rel=B_t_rel(I1); B_phi_rel=B_phi_rel(I1);nw=nw(I);
